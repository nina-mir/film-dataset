...
- `Year` — release year (pandas nullable `Int64`; coerce with `pd.to_numeric(..., errors='coerce')` before numeric comparisons)
- `Locations` — free-text filming location description (string)
- `Fun_Facts` — free-text notes or trivia about the filming (string)
- `Production_Company` — production company name (string)
...
- `Neighborhood` — SF Planning analysis neighborhood (string). May be null for points outside official neighborhood polygons (Alcatraz, Treasure Island, GGNRA, bay locations).
- `Supervisor_District` — SF supervisor district number (pandas nullable `Int64`). May be null for the same reasons as Neighborhood.
- `geometry` — point geometry for the filming location (Shapely Point). May be null for span/range/transit/multi-place location strings that do not resolve to a single point.

**v1 scope note:** Production_Company, Distributor, Neighborhood, and Supervisor_District are present in the GeoDataFrame but are **not** referenced by any predicate the upstream Filter Extractor will emit in v1. Generated code does not need to handle predicates against these columns. They are documented here so the schema is accurate, not because code generation should target them.
...
The `value` key is present for all operators **except** `is_null` and `is_not_null`, which are unary and carry no value. See the operators table below for which ops require a value.
...
| `is_null`        | field absent or empty (Director, Writer only)  | no `value` key                                                    |
| `is_not_null`    | field present and non-empty (Director, Writer only) | no `value` key                                               |
...
- Do not write to the filesystem. Logging is handled by the execution harness. Do not include with `open(...)` blocks anywhere in the generated function. 
...
**`is_null` on a string field (Director, Writer)**

Predicate clause:
```json
{"field": "Director", "op": "is_null", "type": "attribute"}
```
...
director_str = gdf_copy['Director'].astype(str)
mask_director = (
    gdf_copy['Director'].isna() |
    (director_str.str.strip() == '') |
    director_str.str.lower().isin(['none', 'nan', 'null'])
)
```

Notes:
- The predicate leaf has no `value` key. Do not reference `.get('value')` or construct a value literal.
- `is_null` treats four states as absent: pandas NA, empty string after strip, and the stringy-null literals `'none'`, `'nan'`, `'null'` (case-insensitive).
- v1 supports `is_null` only on `Director` and `Writer`. Other fields will not appear with this op.
- This parallels the guarded person-name pattern used for `==`, but inverts the final clause: instead of requiring a substring match, require that all four absence conditions hold.

---

**`is_not_null` on a string field (Director, Writer)**

Predicate clause:
```json
{"field": "Writer", "op": "is_not_null", "type": "attribute"}
```

Pandas mask:
```python
writer_str = gdf_copy['Writer'].astype(str)
mask_writer = (
    gdf_copy['Writer'].notna() &
    writer_str.str.strip().ne('') &
    ~writer_str.str.lower().isin(['none', 'nan', 'null'])
)
```

Notes:
- The predicate leaf has no `value` key.
- This is the complement of `is_null` — the same four absence states, all negated and combined with `&`.
- v1 supports `is_not_null` only on `Director` and `Writer`.
...
- For `is_null` / `is_not_null`: the predicate leaf has no `value` key. Build the mask from the four absence conditions (NA, empty-after-strip, stringy-nulls). v1 scope is Director and Writer only.
